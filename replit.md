...
## Recent Changes
- **Database Schema Fix (Oct 10, 2025)**: Fixed production authentication error by setting explicit PostgreSQL search_path to 'public,auth' in database connection, ensuring queries use the correct users table (public.users) while maintaining Supabase auth compatibility.
